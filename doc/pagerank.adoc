...
If you remember back in the link:#import[import section] we saved the social network as an 'undirected graph'.
Relationships in Neo4j always have a direction but in our graph if `PersonA` is a `FRIEND` with `PersonB` we'll assume that the inverse is true.

At the moment there is no option to treat relationships as undirected but we can use Cypher loading to solve this.


An undirected graph can be represented as a https://en.wikipedia.org/wiki/Bidirected_graph[Bidirected graph]
i.e. a directed graph in which the reverse of every relationship is also a relationship.

We do not have to save these reversed relationships, we can project it using *Cypher loading*.
