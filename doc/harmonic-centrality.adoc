...
* for each node determine the distance from the node to all other nodes

The harmonic centrality for a node is then calculated using the following formula:

`harmonic centrality(node) = sum(1 / distance from node to every other node excluding itself)`

The advantage of using this formula is that ∞ are handled cleanly.
Harmonic centrality and closeness centrality will often come up with similar results, but harmonic centrality can handle graphs that aren't strongly connected. [3]
