...
final class NonWeightedComputeStep implements ComputeStep  {
        ...
        for (int nodeId = startNode; nodeId < endNode; ++nodeId) {
            ...
            if (delta > 0) {
                ...
                if (degree > 0) {
                    ...
                    rels.forEachRelationship(nodeId, Direction.OUTGOING, this::accept);
                }
            }
        }
    }

    private boolean accept(int sourceNodeId, int targetNodeId, long relationId) {
        if (srcRankDelta != 0) {
